...
VBox::VBox(int filename): name(filename) {
    name = filename; // Set the name private variable to filename
    lseek(name,0,SEEK_SET); // Get the offset from the beginning of the file
    read(name, &head, sizeof(head)); // Take the bytes from the file and store them in head
    lseek(name,head.offset_blocks,SEEK_SET);// Get the offset from the file starting at the last part of head
    ...
    std::printf("\n\n\n");
    std::printf("Loaded and read VDI file into head.\n");
    std::printf("Prepared data array & offset, and read vdi bytes into\n");
    std::printf("............ iMap\n\n");
}
